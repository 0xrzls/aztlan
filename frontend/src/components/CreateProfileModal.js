...
const CreateProfileModal = ({ isOpen, onClose, onComplete }) => {
  ...
  // Pick random avatar on first render
  useEffect(() => {
    if (!formData.avatar) {
      setFormData(prev => ({
        ...prev,
    ...
    <div className="fixed inset-0 z-[999] bg-black/70 backdrop-blur-sm flex justify-center items-center">
      <div className="bg-[#121212] rounded-2xl w-[90vw] max-w-md overflow-hidden animate-fade-in">
        ...
        <div className="relative h-40 bg-gradient-to-r from-purple-900/60 to-purple-500/60">
          ...
          <button 
            onClick={onClose} 
            className="absolute top-3 right-3 z-10 bg-black/40 p-2 rounded-full text-white"
            disabled={loading}
          >
            <FaTimes size={16} />
          </button>
          ...
          <div className="absolute -bottom-12 left-1/2 transform -translate-x-1/2">
            <div className="relative">
              ...
              <button 
                ...
                className="absolute bottom-0 right-0 bg-purple-600 p-1.5 rounded-full text-xs text-white font-medium"
                disabled={loading}
              >
                <FaRandom size={12} />
              </button>
        ...
        <div className="pt-16 px-6">
          ...
          {/* Login Option */}
          <div className="bg-[#1f1f1f]/80 rounded-lg p-4 space-y-2 my-4">
            <p className="text-sm text-white/80 flex items-center gap-1">
              Already have Aztlan Account?{' '}
              <button 
                className="text-purple-400 hover:underline flex items-center gap-1"
                disabled={loading}
              >
                <RiLoginCircleLine /> Log in now
              </button>
            </p>
            <p className="text-xs text-white/50 flex items-center gap-2">
              <span className="w-4 h-4 bg-purple-600 rounded-sm flex-shrink-0" />
              <span>Connect with your social to get extra reward points!</span>
            </p>
          </div>
          ...
          <div className="space-y-3 pb-6">
            <button
              className={`w-full py-3 rounded-full text-white text-center font-semibold text-lg 
                ${isValid && !loading 
